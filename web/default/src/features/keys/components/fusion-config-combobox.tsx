...
function fusionConfigLabel(config: FusionConfig) {
  return `${config.name} / ${config.model_alias}`
}

export function FusionConfigCombobox(props: FusionConfigComboboxProps) {
  ...
  const filteredConfigs = useMemo(() => {
    ...
    if (!search) return props.configs
    return props.configs.filter((config) => {
      return (
        config.name.toLowerCase().includes(search) ||
        config.model_alias.toLowerCase().includes(search)
      )
    })
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            className='border-input bg-muted/40 hover:bg-muted/55 hover:text-foreground active:bg-background data-popup-open:border-ring data-popup-open:bg-background data-popup-open:ring-ring/20 h-auto min-h-14 w-full justify-between gap-2 rounded-lg px-3 py-2 text-start shadow-none transition-[background-color,border-color,box-shadow] duration-150 data-popup-open:ring-[3px]'
          />
        }
      >
      ...
      >
        <Command shouldFilter={false}>
          ...
          <CommandList className='max-h-[360px]'>
            <CommandEmpty>{t('No Fusion config found.')}</CommandEmpty>
            <CommandGroup>
              {filteredConfigs.map((config) => (
                <CommandItem
                  key={config.id}
                  value={config.model_alias}
                  onSelect={() => handleSelect(config.model_alias)}
